...
XL_PROD_cotton['PlantedDate'] = pd.to_datetime(XL_PROD_cotton['PlantedDate'], errors='coerce')
XL_PROD_cotton['PD_DOY'] = XL_PROD_cotton['PlantedDate'].dt.dayofyear
...
XL_PROD_cotton['PWId'] = XL_PROD_cotton['PWId'].astype(str).str.strip()
final_df['PWId'] = final_df['PWId'].astype(str).str.strip()


# In[9]:


df_with_PD = final_df.merge(
    XL_PROD_cotton[['PWId', 'ReinsuranceYear', 'PD_DOY']], 
    left_on=['PWId', 'Year'], 
    right_on=['PWId', 'ReinsuranceYear'], 
    how='left'
).drop(columns=['ReinsuranceYear'])

df_with_PD = df_with_PD.drop_duplicates()
...
prism = pd.read_csv(r"F:\globalag\clus_sandwich_test\GEE_golbalag_2159clus_PRISM_stats_JultToDecember_StateCode_48_v2_new_calc.csv")
TX_area_map = dict(zip(prism['PWId'], prism['TX_area']))
PracticeCode_map = dict(zip(prism['PWId'], prism['PracticeCode']))
...
prism.drop(columns=['pptMean_MonthlyMean', 'Unnamed: 0', 'CLUID', 'PracticeCode', 'TX_area', 'yield'], inplace=True)
prism
...
def create_pivot_table(df):
    """
    Transforms the DataFrame to have PWId and Year as identifiers,
    Month as columns, and aggregates other columns accordingly.
    """

    # Columns to aggregate monthly
    monthly_cols = [col for col in df.columns if 'Monthly' in col]

    # Columns to keep as is (annual sums)
    annual_cols = [col for col in df.columns if 'Annual' in col]

    # Columns to keep as identifiers for pivoting
    id_cols_pivot = ['PWId', 'Year']

    # Group by PWId, Year, and Month for monthly aggregation
    ...
    # Reset the index to make PWId and Year regular columns
    monthly_df = monthly_pivot.reset_index()


    # Flatten the multi-level column index of monthly_df AFTER renaming
    new_columns_monthly = []
    for col in monthly_df.columns.values:
        if isinstance(col, tuple):
            new_columns_monthly.append('_'.join(str(item) for item in col).strip())
        else:
            new_columns_monthly.append(str(col).strip())
    monthly_df.columns = new_columns_monthly

    # Select the first row for each PWId and Year for annual data
    annual_df = df.groupby(['PWId', 'Year'])[annual_cols].first().reset_index()
    
     # Explicitly rename the first column to 'PWId' and the second to 'Year'
    if monthly_df.columns[0] != 'PWId':
        monthly_df = monthly_df.rename(columns={monthly_df.columns[0]: 'PWId'})
    if monthly_df.columns[1] != 'Year':
        monthly_df = monthly_df.rename(columns={monthly_df.columns[1]: 'Year'})
    
    print("Columns in monthly_df before merge:", monthly_df.columns)
    print("Columns in annual_df before merge:", annual_df.columns)

    # Merge the monthly and annual data, using the corrected column names
    final_df = monthly_df.merge(
        annual_df, on=['PWId','Year'], how='left'
    ).fillna(
        0
...
transformed_df = create_pivot_table(prism.copy())
transformed_df
...
transformed_df['PWId'] = transformed_df['PWId'].astype(str)
df_with_PD['Year'] = pd.to_numeric(df_with_PD['Year'], errors='coerce').astype('Int64')
transformed_df['Year'] = pd.to_numeric(transformed_df['Year'], errors='coerce').astype('Int64')

# Merge df_with_PD and transformed_df based on 'PWId' and 'Year'
df_with_PD_prism = pd.merge(df_with_PD, transformed_df, on=['PWId', 'Year'], how='left', suffixes=('_left', '_right'))

# Drop the duplicate 'PWId' and 'Year' columns from the right side (transformed_df)
df_with_PD_prism = df_with_PD_prism.drop(columns=['PWId_right', 'Year_right'], errors='ignore')

# Rename the left side columns to their original names (optional, for clarity)
df_with_PD_prism = df_with_PD_prism.rename(columns={'PWId_left': 'PWId', 'Year_left': 'Year'})

# Print the resulting DataFrame (optional)
print(df_with_PD_prism.columns.to_list())
df_with_PD_prism
# df_with_PD_prism.to_csv(r"F:\globalag\clus_sandwich_test\globalag_all_new_calc_indices_with_acreYield_thresholds_PD_prism_v3_singleCluUnit_reshaped.csv")


# # RF with acres and annual production threshold (indices + PD + prism)
# 
...
if __name__ == "__main__":
    ...
    # Set output folder
    output_dir = r"F:\globalag\clus_sandwich_test\v3_rf_outputs_singleCluUnit_PD_prism"
...
# # RF with acres and annual production threshold (only indices and PD)
# 
...
if __name__ == "__main__":
    ...
    # Set output folder
    output_dir = r"F:\globalag\clus_sandwich_test\v3_rf_outputs_singleCluUnit_PD"
...
if __name__ == "__main__":
    ...
    df = df_with_PD_prism  # Replace this with actual data loading if needed

    output_dir = r"F:\globalag\clus_sandwich_test\v3_rf_outputs_singleCluUnit_years_test_PD_prism"
...
stats_df = df_with_PD_prism.copy()  # optional: prevent modifying final_df directly
